...
struct BoolDescriptor : MenuDescriptor {
    ...
    void next(MenuValue v) const override {
        if (v.boolean) *v.boolean = !*v.boolean;
    }
    ...
    const char* format(MenuValue v, char*, size_t) const override {
        if (!v.boolean) return "?"; // valeur non liee (ne devrait pas arriver en usage normal)
...
struct IntDescriptor : MenuDescriptor {
    ...
    void next(MenuValue v) const override {
        if (!v.integer) return;
        *v.integer += step;
        if (*v.integer > max) *v.integer = max; // clamp, pas de wrap circulaire
    }

    void prev(MenuValue v) const override {
        if (!v.integer) return;
        *v.integer -= step;
        if (*v.integer < min) *v.integer = min;
    }
...
struct FloatDescriptor : MenuDescriptor {
    ...
    void next(MenuValue v) const override {
        ...
        *v.floating += step;
        if (*v.floating > max) *v.floating = max;
    }

    void prev(MenuValue v) const override {
        if (!v.floating) return;
        *v.floating -= step;
        if (*v.floating < min) *v.floating = min;
    }
...
struct EnumDescriptor : MenuDescriptor {
    ...
    const char* format(MenuValue v, char*, size_t) const override {
        ...
        for (uint8_t i = 0; i < count; ++i) {
            if (options[i].value == *v.enumeration) return options[i].label;
        }
