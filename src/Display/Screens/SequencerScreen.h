...
public:
    ...
    void draw() override
    {
        ...
        switch (sequencer.playState) {
            ...
            case Sequencer::PlayState::Pause:
                u8g2.drawBox(x, y, 2, 7);
                u8g2.drawBox(x + 4, y, 2, 7);

                break;

            case Sequencer::PlayState::Stop:
        ...
        // Sequencer Volume
        snprintf(buffer, sizeof(buffer), "%3d", sequencer.volume);
        u8g2.drawStr(x, y, buffer);

        // Sequencer BPM
        snprintf(buffer, sizeof(buffer), "%3d", sequencer.bpm);
        u8g2.drawStr(x + (u8g2.getMaxCharWidth() * 3) + u8g2.getMaxCharWidth(), y, buffer);

        // Selected instrument
        snprintf(buffer, sizeof(buffer), "%01X/%01X", uiState.selectedInstrument, audioEngine.instrumentsCount);
        ...
        // Autoscroll
        snprintf(buffer, sizeof(buffer), "%d", uiState.autoScroll);
        ...
        for (uint8_t i = uiState.displayedTrack; i < uiState.displayedTrack + Constants::NUMBER_OF_DISPLAYED_TRACKS;
             i++) {
            // Tracks
            char buffer[Constants::SCREEN_WIDTH / Constants::NUMBER_OF_DISPLAYED_TRACKS / u8g2.getMaxCharWidth() + 1
                        + 7];
            Sequencer::Track& track = sequencer.tracks[i];

            if (i == uiState.selectedTrack) {
                if (uiState.displayTrackInfo) {
                    snprintf(buffer, sizeof(buffer), "%01X %3d %3d", track.instrument, track.volume, track.transpose);
                }
                else {
                    snprintf(buffer, sizeof(buffer), "> Tr%1d", i);
                }

                u8g2.drawStr((i - uiState.displayedTrack) * 60, u8g2.getAscent() + 1 * u8g2.getMaxCharHeight(), buffer);
            }
            else {
                snprintf(buffer, sizeof(buffer), "Tr%1d %01X", i, track.instrument);
                snprintf(buffer, sizeof(buffer), "Tr%1d", i);
            ...
            // Quarter notes
            Sequencer::QuarterNote& qn = sequencer.tracks[i].quarterNotes[uiState.selectedQuarterNote];

            for (uint8_t j = 0; j < qn.stepsCount; j++) {
                Sequencer::Step& step = qn.steps[j];
                ...
                if (step.state) {
                    ...
                    char inst = (step.instrument < 0) ? '-' : "0123456789ABCDEF"[step.instrument];

                    snprintf(buffer, sizeof(buffer), "%3s %c %s", notesStr[step.note], inst, fx);

                    if (uiState.selectedStep == j && uiState.selectedTrack == i) {
                        u8g2.drawButtonUTF8(((i - uiState.displayedTrack) * 60) + 2 + 5, 24 + (j * height),
                                            U8G2_BTN_INV, 0, 1, 1, buffer);
                    }
                    else {
