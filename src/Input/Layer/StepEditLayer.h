    ...
    public:
        ...
        void onEncoder(const InputEvent& event) override
        {
            ...
            if (event.control == ControlId::Encoder0) {
                sequencerTimer.setStepNote(uiState.displayedTrack + row, sequencerTimer.selectedQuarterNote, col, event.value, step.note / 12 - 1);
            }

            if (event.control == ControlId::Encoder1) {
                sequencerTimer.setStepNote(uiState.displayedTrack + row, sequencerTimer.selectedQuarterNote, col, step.note % 12, event.value);
            }
        }
        ...
        void onStepReleased(const InputEvent& inputEvent) override
        {
            ...
            sequencerTimer.toggleStep(uiState.displayedTrack + row, sequencerTimer.selectedQuarterNote, col);    
        }
};
